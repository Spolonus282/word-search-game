...
import pygame, PlayersGui, Check, Input, Utilities, Game, NewDictionary, NewMenu, MultiGame
...
NewMenu.Rght = Rght
NewMenu.Nums = Nums
NewDictionary.Back = Back
Utilities.DISPSURF = DISPSURF
PlayersGui.DISPSURF = DISPSURF
NewDictionary.DISPSURF = DISPSURF
NewMenu.DISPSURF = DISPSURF
Utilities.File = File
...
NewDictionary.Pending = Pending
NewMenu.Pending = Pending
Utilities.Pend = Pend
NewDictionary.Pend = Pend
Game.Pend = Pend
MultiGame.Pend = Pend
...
while True:
    ...
    NewMenu.Pend = Pend
    startCheck = ''
    startCheck = NewMenu.NewMenu(dispScore, Player[4])
    if startCheck == 'dict':
        Utilities.Update([], True, (0, 75, xSize, ySize - 40))
        if file2[0] in ['Sam', 'test']:
            Wording = NewNewDictionary.adminAccept(Pend)
            Pend = Wording[0]
            Worded = Wording[1]
            if not Wording[2]:
                Utilities.Update([], True, (0, 75, xSize, ySize - 40))
                Pend += NewNewDictionary.AddWords()
            while '' in Pend:
                del Pend[Pend.index('')]
        else:
            Pend += NewDictionary.AddWords()
        Utilities.Pend = Pend
        Game.Pend = Pend
        MultiGame.Pend = Pend
        NewDictionary.Pend = Pend
